...
public class MockAdminService : IAdminService
{
    ...
    public Task<JaxSun.Ideas.Core.DTOs.Auth.AdminStatsResponse?> GetStatsAsync()
    {
        _logger.LogInformation("Mock admin service: Getting admin stats");
        
        var stats = new JaxSun.Ideas.Core.DTOs.Auth.AdminStatsResponse
        {
            TotalUsers = _mockUsers.Count,
            VerifiedUsers = _mockUsers.Count(u => u.IsVerified)
        };
        
        return Task.FromResult<JaxSun.Ideas.Core.DTOs.Auth.AdminStatsResponse?>(stats);
    }
}